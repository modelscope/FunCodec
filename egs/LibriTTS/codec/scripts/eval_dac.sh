...
if [ ${stage} -le 4 ] && [ ${stop_stage} -ge 4 ]; then
    ...
    for dset in ${test_sets}; do
        ...
        ${infer_cmd} --gpu "${_ngpu}" --max-jobs-run "${_nj}" JOB=1:"${_nj}" "${_logdir}"/encode.JOB.log \
            python3 -m dac encode ${_logdir}/keys.JOB.scp \
              --weights_path /home/neo.dzh/.cache/descript/24khz/0.0.4/ \
              --output ${_dir}/encode/output.JOB/ \
              --device cuda:JOB \
              --n_quantizers ${nq}

        echo "Decoding for $dset nq=${nq}"
        ${infer_cmd} --gpu "${_ngpu}" --max-jobs-run "${_nj}" JOB=1:"${_nj}" "${_logdir}"/decode.JOB.log \
            python3 -m dac decode ${_dir}/encode/output.JOB/ \
              --weights_path /home/neo.dzh/.cache/descript/24khz/0.0.4/ \
              --output ${_dir}/decode/output.JOB/ \
              --device cuda:JOB \

...
def tree_layer_info(
        ...
        layer_info: dict,
        depth=0,
        ignore_list=["quantizer", "discriminator", "enc_trans_func", "dec_trans_func", "mel_spec_transforms"]
) -> str:
    if len(layer_info) == 0:
        return ""
    ...
    for key, value in layer_info.items():
        if key in ignore_list:
            continue
        cur_flop, cur_param = value[:2]
        clv_flop, clv_param = clever_format([cur_flop, cur_param], "%.2f")
        ret_str += "{}{}: flops {}({:.2f}%), param. {}({:.2f}%)\n".format(
            "  " * depth, key,
            clv_flop, (cur_flop/total_flops*100.0),
            clv_param, (cur_param/total_params*100.0),
        )

        ret_str += tree_layer_info(total_flops, total_params, value[2], depth+1)
